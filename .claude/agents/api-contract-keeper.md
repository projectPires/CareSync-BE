...
description: "Use this agent when adding/changing any HTTP endpoint, DTO, response shape, status code, or OpenAPI annotation. Owns the /v1 contract that the mobile repo (projectPires/CareSync) codegens its typed client from. Detects breaking changes; enforces versioning and error-shape conventions."
model: sonnet
---
...
1. **Everything under `/v1`.** New breaking shapes → new endpoint or `/v2`, never mutate `/v1` semantics.
...
- Plural resources, kebab-case paths: `/v1/residents/:id/medications`.
